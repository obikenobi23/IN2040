...
;;1c:
;;Prosedyren husker kun den direkte løsningen for det ene kallet, her (fib 3) men ikke de andre kallene som krevdes for å få svaret.
;;I tabellen kan man antakelig slå opp på hva svaret av (fib 3) er, men svaret av (fib 2), som man trengte for å finne (fib 3) ble ikke lagt inn
;;under utregningen av (fib 3), slik som man ønsket.
...
;;stream-to-list:
;;Vi antar at en ikke trenger å sette en grunneleggende grense for hvor mange elementer som kan konverteres,
;;men at det valgfrie argumentet er nok for å ta høyde for dette.
;;Dersom det forventes at en tar høyde for at den kan være uendelig, ville vi satt inn at det ikke skal kunne konverteres mer enn
;;f eks 1000 elementer, og det kunne vi gjort ved å sende inn 1000 som siste argument til prosedyren, i else-grenen til if, siste linje
;;av stream-to-list.
(define (stream-to-list list-to-convert . args)
  (cond ((stream-null? list-to-convert) '()) ;;Dersom strømmen er tom, returner den tomme listen
        ((and (not (null? args)) (zero? (car args))) '()) ;;Dersom args ikke er tom, og nedtellingen, n, har nådd 0, returneres tom liste. 
        (else
