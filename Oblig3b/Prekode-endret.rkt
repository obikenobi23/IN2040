...
(define (eval-special-form exp env)
  (begin (cond ((quoted? exp) (text-of-quotation exp))
               ((assignment? exp) (eval-assignment exp env))
               ((definition? exp) (eval-definition exp env))
               ((if-then? exp) (eval-if-then exp env));3b
               ...
               ((and? exp) (eval-and (cdr exp)));3a
               ((or? exp) (eval-or (cdr exp)));3a
               ((let? exp) (eval-let (cdr exp) env)))));3c og 3d
...
(define (eval-if-then exp env)
  (if (true? (mc-eval (if-predicate exp) env))
      (mc-eval (if-then-consequent exp) env)
      (let ((next-branch (cddddr exp)))
        (if (elsif? next-branch) (eval-if-then next-branch env)
            (if (else? next-branch) (mc-eval (cadr next-branch) env)
                (display "Syntaks-feil: if-statement med then"))))))

;3b
(define (elsif? exp)
  (if (tagged-list? exp 'elsif) #t #f))

(define (else? exp)
  (if (tagged-list? exp 'else) #t #f))
...
;3a
;; And og or i denne evaluatoren kan ta ett eller null argumenter. Jeg designet det slik med vilje, tror det kan ha potensiale som en kul "exploit" i fremtiden.
(define (eval-and exp)
  (if (null? exp) #t
      (if (true? (car exp))
...
(define (eval-or exp)
  (if (null? exp) #f
      (if (true? (car exp)) (car exp)
          (eval-or (cdr exp)))))

(define (find-vars let-args);Argumentlisten til let ()
  (if (null? let-args) '()
      (cons (caar let-args) (find-vars (cdr let-args)))))

(define (find-exps let-args);Argumentlisten til let ()
  (if (null? let-args) '()
      (cons (cadar let-args) (find-exps (cdr let-args)))))

;; Gjorde en endring i make-lambda for å passe inn expressions i en egen binding: Der make-lambda kalles i prekoden har jeg gitt dette argumentet som '()
(define (eval-let exp env)
  (let ((var-list (car exp)) (body (cdr exp)))
    (mc-eval
     (make-lambda (find-vars var-list) body (find-exps var-list)) env)))


;;Special-let 3d
(define (find-variables-special exp)
  (cond ((eq? 'in (cadddr exp))
        (cons (car exp) '()))
       (else (begin (display (cons "conser " (car exp))) (cons (car exp) (find-variables-special (cddddr exp)))))))


(define (find-expressions-special exp)
  (cond ((eq? 'in (cadddr exp))
        (cons (caddr exp) '()))
       (else (begin (display (cons "conser " (caddr exp))) (cons (caddr exp) (find-expressions-special (cddddr exp)))))))


(define (find-body exp)
  (if (eq? 'in (car exp))
      (begin (display exp)
      (cdr exp)) ;;returner exp, da er det body
      (find-body (cdr exp))))


;(define (eval-let-special exp env)
;  (mc-eval (cons (make-lambda (find-variables (cdr exp)) (find-body (cdr exp))) (find-expressions (cdr exp))) env)) ;;cdr exp og cdr exp for å fjerne let
  ;;kan teste de enklete prosedyrene, se hva som returnereres, ved åå bytte ut uttrykket over med enten findbody, find-expressions eller find-variables.
...
(define (make-lambda parameters body expressions)
  (cons (cons 'lambda (cons parameters body)) expressions))
...
(define (if-then? exp)
  (if (null? exp) #f
      (if (tagged-list? exp 'then) #t (if-then? (cdr exp)))))

(define (if-predicate exp) (cadr exp))

(define (if-consequent exp) (caddr exp))

(define (if-then-consequent exp) (cadddr exp))
...
(define (let? exp) (tagged-list? exp 'let));3c
...
;Prosedyredefinisjon for 2a
(define (1+ addend)
  (+ addend 1))
(define (1- minuend)
  (- minuend 1))

;(define (install-primitive! proc-symbol definition)
;  (append primitive-procedures (list proc-symbol definition)))

;Prosedyredefinisjon for 2b
(define (install-primitive! primitive-name primitive)
  (set! primitive-procedures (cons primitive-procedures (list (list primitive-name primitive)))) ;;primtive-name er quotet allerede antar vi
  (set! the-global-environment (setup-environment)))
...
(define primitive-procedures
  (list (list 'car car)
        ...
        (list '1+ 1+);2a
        (list '1- 1-);2a
        (list 'install-primitive! install-primitive!);2b
        ))
